...
Hooks.on('preCreateChatMessage', (msg, options, userId) => {
  ...
  if(msg.data.flags){
    skillId = msg.data.flags.wwnpretty?.roll?.skillId;
    saveId = msg.data.flags.wwnpretty?.roll?.saveId;
    isInitiative = msg.data.flags.core?.initiativeRoll;
    // isDeathSave = msg.data.flags.wwnpretty?.roll?.type === "death";
    isAttack = msg.data.flags.wwnpretty?.roll?.type === "attack";
  }

  // Blind-ify Default 5e & MARS 5e Roll Cards
  // We check the wwnpretty flags in the msg to see if it contains any of the skills,
