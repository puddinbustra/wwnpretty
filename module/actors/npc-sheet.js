...
export class NPCActorSheet extends ActorSheet {
    ...
    getData() {
        ...
        return mergeObject(data, {
            ...
            abilities: this.actor.items.filter((i) => ["power", "focus"].indexOf(i.type) !== -1),
            equipment: this.actor.items.filter((i) => ["armor", "item", "weapon"].indexOf(i.type) !== -1),
        });
    }
    ...
    async _onItemDamage(event) {
        ...
        const template = await renderTemplate("systems/swnr/templates/dialogs/roll-attack.html", { actor: this.actor.data, weapon, burstFireHasAmmo: weapon.canBurstFire });
        const doRoll = async (html) => {
            var _a, _b;
            ...
            const burstMode = (_b = (_a = html.find('[name="burstFire"]')) === null || _a === void 0 ? void 0 : _a.prop("checked")) !== null && _b !== void 0 ? _b : false;
    ...
    async _onReaction(event) {
        ...
        const diceSoNiceDisableState = (_a = game.dice3d) === null || _a === void 0 ? void 0 : _a.messageHookDisabled;
        if (game.dice3d) {
            ...
            Hooks.once("preCreateChatMessage", (chat) => {
                console.log(chat);
                if (Object.keys(chat.flags).includes("core.RollTable") &&
        ...
        if (game.dice3d)
            game.dice3d.messageHookDisabled = diceSoNiceDisableState;
    ...
    _onMorale(event) {
        ...
        console.log(roll);
        const flavor = roll.results[0] > this.actor.data.data.moralScore
    ...
    _onSavingThrow(event) {
        ...
        const flavor = game.i18n.format(parseInt(roll.result) >= this.actor.data.data.saves
            ? game.i18n.localize("swnr.npc.saving.success")
            : game.i18n.localize("swnr.npc.saving.failure"), { actor: this.actor.name });
        roll.toMessage({ flavor, speaker: { actor: this.actor._id } });
    }
    _onSkill(event) {
        event.preventDefault();
