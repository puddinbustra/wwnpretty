...
export class SWNRCharacterActor extends Actor {
    getRollData() {
    ...
    prepareDerivedData() {
        ...
        const baseAc = Math.max(data.baseAc, ...armor.map((i) => i.data.data.ac +
            (shields.filter((s) => s.id !== i.id).length !== 0 ? 1 : 0)));
        data.ac = baseAc + data.stats.dex.mod;
        // effort
        const psychicSkills = (this.items.filter((i) => i.type === "skill" &&
