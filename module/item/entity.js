...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.WWNPRETTY.areaTargetTypes);
  }
  ...
  prepareDerivedData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("WWNPRETTY.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("WWNPRETTY.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "WWNPRETTY.Attack" : "WWNPRETTY.Action");
      else labels.featType = game.i18n.localize("WWNPRETTY.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("WWNPRETTY.AC")}` : "";
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("WWNPRETTY.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.WWNPRETTY.abilities[save.ability];
    this.labels.save = game.i18n.format("WWNPRETTY.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
  ...
  async roll({configureDialog=true, rollMode, createMessage=true}={}) {
    ...
    // const requireSpellSlot = isSpell && (id.level > 0) && CONFIG.WWNPRETTY.spellUpcastModes.includes(id.preparation.mode);
    ...
    if ( createMeasuredTemplate ) {
      const template = game.wwnpretty.canvas.AbilityTemplate.fromItem(item);
  ...
  _getUsageUpdates({consumeQuantity, consumeRecharge, consumeResource, consumeSpellLevel, consumeUsage}) {
    ...
    if ( consumeRecharge ) {
      ...
      if ( recharge.charged === false ) {
        ui.notifications.warn(game.i18n.format("WWNPRETTY.ItemNoUses", {name: this.name}));
        return false;
      }
    ...
    if ( consumeSpellLevel ) {
      ...
      if ( spells === 0 ) {
        const label = game.i18n.localize(consumeSpellLevel === "pact" ? "WWNPRETTY.SpellProgPact" : `WWNPRETTY.SpellLevel${id.level}`);
        ui.notifications.warn(game.i18n.format("WWNPRETTY.SpellCastNoSlots", {name: this.name, level: label}));
        return false;
      }
  ...
  _handleConsumeResource(itemUpdates, actorUpdates, resourceUpdates) {
    ...
    // No consumed target
    const typeLabel = CONFIG.WWNPRETTY.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("WWNPRETTY.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( !resource ) {
      ui.notifications.warn(game.i18n.format("WWNPRETTY.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("WWNPRETTY.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/wwnpretty/templates/chat/item-card.html", templateData);

    // // Render the chat card template
    // const templateType = ["tool"].includes(this.data.type) ? this.data.type : "item";
    // const template = `systems/wwnpretty/templates/chat/${templateType}-card.html`;
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["wwnpretty.itemData"] = this.data;
    }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("carried") && ["weapon"].includes(this.data.type) && data.magsize ) {
      props.push(
        ...
        // game.i18n.localize(data.proficient ? "WWNPRETTY.Proficient" : "WWNPRETTY.NotProficient")
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.WWNPRETTY.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("WWNPRETTY.StealthDisadvantage") : null
    );
  }
  ...
  //   props.push(
  //     CONFIG.WWNPRETTY.weaponTypes[data.weaponType],
  //   );
  // }
  ...
  //   props.push(
  //     CONFIG.WWNPRETTY.consumableTypes[data.consumableType],
  //     data.uses.value + "/" + data.uses.max + " " + game.i18n.localize("WWNPRETTY.Charges")
  //   );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.WWNPRETTY.abilities[data.ability] || null,
      CONFIG.WWNPRETTY.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("WWNPRETTY.ItemTypeLoot"),
      data.weight ? data.weight + " " + game.i18n.localize("WWNPRETTY.AbbreviationLbs") : null
    );
  }
  ...
  async rollAttack(options={}) {
    const itemData = this.data.data;
    const flags = this.actor.data.flags.wwnpretty || {};
    if ( !this.hasAttack ) {
      throw new Error("You may not place an Attack Roll with this Item.");
    }

    let title = `${this.name} - ${game.i18n.localize("WWNPRETTY.AttackRoll")}`;
    ...
    if(this.data.data.shockDam){
      ...
      title = `${this.name} - ${game.i18n.localize("WWNPRETTY.AttackRoll")} - Shock: ${this.data.data.shockDam}`;
    }
    ...
    let rollConfig = {
      ...
      messageData: {"flags.wwnpretty.roll": {type: "attack", itemId: this.id }}
    };
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const messageData = {"flags.wwnpretty.roll": {type: "damage", itemId: this.id }};
    ...
    const actionFlavor = game.i18n.localize(itemData.actionType === "heal" ? "WWNPRETTY.Healing" : "WWNPRETTY.DamageRoll");
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.wwnpretty.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("wwnpretty", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula(options={}) {
    ...
    // const title = `${this.name} - ${game.i18n.localize("WWNPRETTY.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.wwnpretty.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("WWNPRETTY.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "WWNPRETTY.ItemRechargeSuccess" : "WWNPRETTY.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("WWNPRETTY.ToolCheck")}`;
    ...
    const rollConfig = mergeObject({
      ...
      chooseModifier: true,
      // halflingLucky: this.actor.getFlag("wwnpretty", "halflingLucky" ) || false,
      // reliableTalent: (this.data.data.proficient >= 1) && this.actor.getFlag("wwnpretty", "reliableTalent"),
      messageData: {"flags.wwnpretty.roll": {type: "tool", itemId: this.id }}
    }, options);
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("wwnpretty", "itemData");
    const item = storedData ? new this(storedData, {parent: actor}) : actor.items.get(card.dataset.itemId);
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("WWNPRETTY.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}))
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("WWNPRETTY.ActionWarningNoToken"));
    return targets;
  }
  ...
  _onCreateOwnedEquipment(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        // const armorProf = CONFIG.WWNPRETTY.armorProficienciesMap[data.data?.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        // const weaponProf = CONFIG.WWNPRETTY.weaponProficienciesMap[data.data?.weaponType]; // Player characters check proficiency
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.WWNPRETTY.sourcePacks.ITEMS}.${CONFIG.WWNPRETTY.spellScrollIds[level]}`;
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("WWNPRETTY.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
