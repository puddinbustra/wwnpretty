...
export default class D20Roll extends Roll {
  constructor(formula, data, options) {
    ...
    //I'm commenting this out so I can use this function for non-d20 rolls -Lofty
   ...
   * @type {string}
   */
  static EVALUATION_TEMPLATE = "systems/wwnpretty/templates/chat/roll-dialog.html";
  ...
  async toMessage(messageData={}, options={}) {
    ...
    // Add appropriate advantage mode message flavor and WWN roll flags
    messageData.flavor = messageData.flavor || this.options.flavor;
    if ( this.hasAdvantage ) messageData.flavor += ` (${game.i18n.localize("WWNPRETTY.Advantage")})`;
    else if ( this.hasDisadvantage ) messageData.flavor += ` (${game.i18n.localize("WWNPRETTY.Disadvantage")})`;
  ...
  async configureDialog({title, defaultRollMode, defaultAction=D20Roll.ADV_MODE.NORMAL, chooseModifier=false, defaultAbility, template}={}, options={}) {
    ...
    const content = await renderTemplate(template ?? this.constructor.EVALUATION_TEMPLATE, {
      ...
      abilities: CONFIG.WWNPRETTY.abilities
    });
  ...
  _onDialogSubmit(html, advantageMode) {
    ...
    if ( form.ability?.value ) {
      ...
      this.options.flavor += ` (${CONFIG.WWNPRETTY.abilities[form.ability.value]})`;
    }
