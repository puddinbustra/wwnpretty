...
export default class ProficiencySelector extends TraitSelector {
  ...
  async getData() {
    ...
    this.options.choices = CONFIG.WWNPRETTY[`${this.options.type}Proficiencies`];
    const data = super.getData();

    // const pack = game.packs.get(CONFIG.WWNPRETTY.sourcePacks.ITEMS);
    const ids = CONFIG.WWNPRETTY[`${this.options.type}Ids`];
    const map = CONFIG.WWNPRETTY[`${this.options.type}ProficienciesMap`];
    if ( ids !== undefined ) {
