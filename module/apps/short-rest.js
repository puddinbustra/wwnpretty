...
export default class ShortRestDialog extends Dialog {
  ...
  /** @override */
	static get defaultOptions() {
	  return mergeObject(super.defaultOptions, {
	    template: "systems/wwnpretty/templates/apps/short-rest.html",
      classes: ["WWNPRETTY", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("WWNPRETTY", "restVariant");
  ...
  static async shortRestDialog({actor}={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: game.i18n.localize("WWNPRETTY.ShortRest"),
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("WWNPRETTY.Rest"),
            callback: html => {
              let newDay = false;
              if (game.settings.get("WWNPRETTY", "restVariant") === "gritty")
