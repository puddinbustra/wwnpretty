...
export default class SelectItemsPrompt extends Dialog {
  constructor(items, dialogData={}, options={}) {
    ...
    this.options.classes = ["WWNPRETTY", "dialog", "select-items-prompt", "sheet"];
  ...
  static async create(items, {
    ...
    const html = await renderTemplate("systems/wwnpretty/templates/apps/select-items-prompt.html", {items, hint});

    return new Promise((resolve) => {
      const dlg = new this(items, {
        title: game.i18n.localize('WWNPRETTY.SelectItemsPromptTitle'),
        content: html,
        buttons: {
          apply: {
            icon: `<i class="fas fa-user-plus"></i>`,
            label: game.i18n.localize('WWNPRETTY.Apply'),
            callback: html => {
          ...
          cancel: {
            ...
            label: game.i18n.localize('WWNPRETTY.Skip'),
            callback: () => resolve([])
          }
        },
        default: "apply",
