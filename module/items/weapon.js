...
export class SWNRWeapon extends SWNRBaseItem {
    ...
    async rollAttack(damageBonus, stat, skillMod, modifier, useBurst) {
        ...
        const hitRoll = new Roll("1d20 + @burstFire + @modifier + @actor.ab + @weapon.ab + @stat + @effectiveSkillRank", rollData).roll();
        rollData.hitRoll = hitRoll.dice[0].total;
        const damageRoll = new Roll(this.data.data.damage + " + @burstFire + @stat + @damageBonus", rollData).roll();
        const diceTooltip = {
        ...
        promise.then(() => {
            ...
            CONFIG.ChatMessage.entityClass.create({
                speaker: ChatMessage.getSpeaker({ actor: (_a = this.actor) !== null && _a !== void 0 ? _a : undefined }),
                // roll: roll,
                content: chatContent,
            }, { rollMode });
        });
