...
export const migrateWorld = async function() {
  ...
  for ( let a of game.actors.contents ) {
    try {
      ...
      err.message = `Failed wwnpretty system migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items.contents ) {
    try {
      ...
      err.message = `Failed wwnpretty system migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes.contents ) {
    try {
      ...
      err.message = `Failed wwnpretty system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("wwnpretty", "systemMigrationVersion", game.system.data.version);
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed wwnpretty system migration for entity ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
export const migrateArmorClass = async function(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.WWNPRETTY.armorTypes));
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.WWNPRETTY.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if ( actorData.flags.wwnpretty ) {
    actorData.flags.wwnpretty = filterObject(actorData.flags.wwnpretty, allowedFlags);
  }
...
function _migrateActorSenses(actor, updateData) {
  ...
  for ( let s of original.split(",") ) {
    ...
    if ( type in CONFIG.WWNPRETTY.senses ) {
...
function _migrateActorType(actor, updateData) {
  ...
  if ( match ) {
    ...
    const typeMatch = Object.entries(CONFIG.WWNPRETTY.creatureTypes).find(([k, v]) => {
      return (typeLc === k) ||
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("WWNPRETTY.CreatureSwarm"));
    if ( match.groups.size || isNamedSwarm ) {
      const sizeLc = match.groups.size ? match.groups.size.trim().toLowerCase() : "tiny";
      const sizeMatch = Object.entries(CONFIG.WWNPRETTY.actorSizes).find(([k, v]) => {
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.WWNPRETTY.attunementTypes.NONE;
...
//   const rarity = Object.keys(CONFIG.WWNPRETTY.itemRarity).find(key =>
//     (CONFIG.WWNPRETTY.itemRarity[key].toLowerCase() === item.data.rarity.toLowerCase()) || (key === item.data.rarity)
//   );
...
export async function purgeFlags(pack) {
  const cleanFlags = (flags) => {
    const flags5e = flags.wwnpretty || null;
    return flags5e ? {wwnpretty: flags5e} : {};
  };
  await pack.configure({locked: false});
