...
export default class ActorSheet5e extends ActorSheet {
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/wwnpretty/templates/actors/limited-sheet.html";
    return `systems/wwnpretty/templates/actors/${this.actor.data.type}-sheet.html`;
  }
  ...
  getData(options) {
    ...
    //   abl.hover = CONFIG.WWNPRETTY.proficiencyLevels[abl.proficient];
    //   abl.label = CONFIG.WWNPRETTY.abilities[a];
    // }
    ...
    if (actorData.data.skills) {
      for ( let [s, skl] of Object.entries(actorData.data.skills)) {
        skl.ability = CONFIG.WWNPRETTY.abilityAbbreviations[skl.ability];
        skl.icon = this._getProficiencyIcon(skl.value);
        skl.hover = CONFIG.WWNPRETTY.proficiencyLevels[skl.value];
        skl.label = CONFIG.WWNPRETTY.skills[s];
  ...
  _getMovementSpeed(actorData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("WWNPRETTY.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("WWNPRETTY.MovementClimb")} ${movement.climb}`],
      [movement.drive, `${game.i18n.localize("WWNPRETTY.MovementDrive")} ${movement.drive}`],
      [movement.fly, `${game.i18n.localize("WWNPRETTY.MovementFly")} ${movement.fly}` + (movement.hover ? ` (${game.i18n.localize("WWNPRETTY.MovementHover")})` : "")],
      [movement.swim, `${game.i18n.localize("WWNPRETTY.MovementSwim")} ${movement.swim}`]
    ]
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("WWNPRETTY.MovementWalk")} ${movement.walk}`]);
    }
  ...
  _getSenses(actorData) {
    ...
    const tags = {};
    for ( let [k, label] of Object.entries(CONFIG.WWNPRETTY.senses) ) {
      const v = senses[k] ?? 0
      if ( v === 0 ) continue;
  ...
  _prepareArmorClassAttribution(data) {
	...
	const cfg = CONFIG.WWNPRETTY.armorClasses[ac.calc];
	const attribution = [];
    ...
    // Flat
		case "flat":
			return [{
				label: game.i18n.localize("WWNPRETTY.ArmorClassFlat"),
		...
		case "natural":
			attribution.push({
				label: game.i18n.localize("WWNPRETTY.ArmorClassNatural"),
				mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
				value: ac.flat
			});
			break;
		...
		case "default":
			...
			attribution.push({
				label: hasArmor ? this.actor.armor.name : game.i18n.localize("WWNPRETTY.ArmorClassUnarmored"),
				mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
				value: hasArmor ? this.actor.armor.data.data.armor.value : 10				
			});
			console.log(game.i18n.localize("WWNPRETTY.ArmorClassUnarmored"));
			if (ac.dex !== 0) {
				attribution.push({
					label: game.i18n.localize("WWNPRETTY.AbilityDex"),
		...
		default:
			...
			attribution.unshift({
				label: game.i18n.localize("WWNPRETTY.PropertyBase"),
				mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
				value: base
			});
			break;
	...
	if( ac.shield !== 0 ) attribution.push({
		label: this.actor.shield?.name ?? game.i18n.localize("WWNPRETTY.EquipmentShield"),
		mode: CONST.ACTIVE_EFFECT_MODES.ADD,
		value: ac.shield
    });
	...
	if ( ac.cover !== 0 ) attribution.push({
      label: game.i18n.localize("WWNPRETTY.Cover"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.cover
    });
    return attribution;
  ...
  _prepareTraits(traits) {
    const map = {
      "dr": CONFIG.WWNPRETTY.damageResistanceTypes,
      "di": CONFIG.WWNPRETTY.damageResistanceTypes,
      "dv": CONFIG.WWNPRETTY.damageResistanceTypes,
      "ci": CONFIG.WWNPRETTY.conditionTypes,
      "languages": CONFIG.WWNPRETTY.languages,
      "armorProf": CONFIG.WWNPRETTY.armorProficiencies,
      "weaponProf": CONFIG.WWNPRETTY.weaponProficiencies,
      "toolProf": CONFIG.WWNPRETTY.toolProficiencies
    };
  ...
  _prepareSpellbook(data, spells) {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.WWNPRETTY.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.WWNPRETTY.spellLevels[lvl], levels[sl]);
      }
    }

    // Pact magic users have cantrips and a pact magic section
    if ( levels.pact && levels.pact.max ) {
      if ( !spellbook["0"] ) registerSection("spell0", 0, CONFIG.WWNPRETTY.spellLevels[0]);
      const l = levels.pact;
      const config = CONFIG.WWNPRETTY.spellPreparationModes.pact;
      const level = game.i18n.localize(`WWNPRETTY.SpellLevel${levels.pact.level}`);
    ...
    spells.forEach(spell => {
      ...
      if ( mode in sections ) {
        ...
        if ( !spellbook[s] ){
          const l = levels[mode] || {};
          const config = CONFIG.WWNPRETTY.spellPreparationModes[mode];
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.WWNPRETTY.spellLevels[s], {levels: levels[sl]});
      }
  ...
  async _onDropActor(event, data) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get('wwnpretty', 'allowPolymorphing'));
    ...
    const rememberOptions = html => {
      ...
      const settings = mergeObject(game.settings.get('wwnpretty', 'polymorphSettings') || {}, options);
      game.settings.set('wwnpretty', 'polymorphSettings', settings);
      return settings;
    };

    // Create and render the Dialog
    return new Dialog({
      title: game.i18n.localize('WWNPRETTY.PolymorphPromptTitle'),
      content: {
        options: game.settings.get('wwnpretty', 'polymorphSettings'),
        i18n: WWNPRETTY.polymorphSettings,
      ...
      buttons: {
        accept: {
          ...
          label: game.i18n.localize('WWNPRETTY.PolymorphAcceptSettings'),
          callback: html => this.actor.transformInto(sourceActor, rememberOptions(html))
        },
        wildshape: {
          icon: '<i class="fas fa-paw"></i>',
          label: game.i18n.localize('WWNPRETTY.PolymorphWildShape'),
        ...
        polymorph: {
          ...
          label: game.i18n.localize('WWNPRETTY.Polymorph'),
          callback: html => this.actor.transformInto(sourceActor, {
            transformTokens: rememberOptions(html).transformTokens
          })
        },
      ...
      classes: ['dialog', 'wwnpretty'],
      width: 600,
      template: 'systems/wwnpretty/templates/apps/polymorph-prompt.html'
    }).render(true);
  }
  ...
  async _onDropItemCreate(itemData) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      return ui.notifications.warn(game.i18n.format("WWNPRETTY.ActorWarningInvalidItem", {
    ...
    if ( itemData.data ) {
      ...
      itemData.data.attunement = Math.min(itemData.data.attunement, CONFIG.WWNPRETTY.attunementTypes.REQUIRED);
    }
  ...
  _onItemCreate(event) {
    ...
    const itemData = {
      name: game.i18n.format("WWNPRETTY.ItemNew", {type: game.i18n.localize(`WWNPRETTY.ItemType${type.capitalize()}`)}),
      type: type,
  ...
  /**
   * Handle rolling a Skill check
   * @param {Event} event   The originating click event, Lofty
   * @private
   */
  _onRollAbilitySave(event) {
    event.preventDefault();
    const save = event.currentTarget.dataset.saves;
    return this.actor.rollSaveWwn(save, {event: event});
  }
  ...
  _onTraitSelector(event) {
    ...
    const a = event.currentTarget;
    const label = a.parentElement.querySelector("label");
    const choices = CONFIG.WWNPRETTY[a.dataset.options];
  ...
  _getHeaderButtons() {
    ...
    if ( this.actor.isPolymorphed ) {
      buttons.unshift({
        label: 'WWNPRETTY.PolymorphRestoreTransformation',
